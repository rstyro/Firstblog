$(function(){
	var pw = $('#my-skills').parent().width();
	var ph = pw*0.9;
  $('#my-skills').radarChart({
    size: [pw, ph],
    step: 1,
    title: "",
    values: {
     ...
     // var spacing = 15;
      ...
      for(var i = min; i <= max; i += this.settings.step) {
        ...
        this.cxt.arc(this.width/2,
                     ...
                     0, 2 * Math.PI, false);
        this.cxt.strokeStyle = "green";	//圆圈的颜色
        this.cxt.fillStyle = 'green';		//坐标的颜色
        this.cxt.stroke();
      ...
      $.each(this.settings.values, function(key,val){
        ...
        that.cxt.fillStyle = "rgba(0,255,255,.8)"; 	//字体颜色
        that.cxt.strokeStyle = "rgba(0,255,0,.5)";	//线的颜色 
